...
class Schwab:
    ...
    def initialConnect(self):
        ...
        self.logger.info(
            f"CONNECTING {self.user['Name']} TO SCHWAB ({modifiedAccountID(self.account_id)})", extra={'log': False})
        ...
        if isValid:
            ...
            self.logger.info(
                f"CONNECTED {self.user['Name']} TO SCHWAB ({modifiedAccountID(self.account_id)})", extra={'log': False})
            ...
            self.logger.error(
                f"FAILED TO CONNECT {self.user['Name']} TO SCHWAB ({self.account_id})", extra={'log': False})
    ...
    def checkTokenValidity(self):
        ...
        if age_sec >= user["Accounts"][self.account_id]['expires_in'] - 60:
            ...
            if token_data:
                ...
                self.users.update_one({"Name": self.user["Name"]}, {
                    "$set": {f"Accounts.{str(self.account_id)}.expires_in": token_data[2]['expires_in'], f"Accounts.{str(self.account_id)}.access_token": token_data[2]["access_token"], f"Accounts.{str(self.account_id)}.created_at": time.time()}})
        ...
        refresh_exp = datetime.strptime(
            user["Accounts"][str(self.account_id)]["refresh_exp_date"], "%Y-%m-%d")
        ...
        if days_left <= 5:
            ...
            if token_data:
                ...
                # ADD NEW TOKEN DATA TO USER DATA IN DB
                self.users.update_one({"Name": self.user["Name"]}, {
                    "$set": {f"{str(self.account_id)}.refresh_token": token_data[2]['refresh_token'], f"{str(self.account_id)}.refresh_exp_date": (datetime.now().replace(
    ...
    def getNewTokens(self, token, refresh_type="Access Token"):
        ...
        resp = requests.post('https://api.schwabapi.com/v1/oauth/authorize?',
                             headers={
        ...
        if resp.status_code != 200:
            ...
            if self.invalid_count == 5:
                ...
                msg = f"{__class__.__name__} - {self.user['Name']} - SCHWAB INSTANCE TERMINATED - {resp.json()} - Refresh Type: {refresh_type} {modifiedAccountID(self.account_id)}"
    ...
    def getAccount(self):
        ...
        url = f"https://api.schwabapi.com/v1/accounts/{self.account_id}?fields={fields}"
    ...
    def getQuotes(self, symbols):
        ...
        url = f"https://api.schwabapi.com/v1/marketdata/quotes?symbol={seperated_values}"
    ...
    def getSpecificOrder(self, id):
        ...
        url = f"https://api.schwabapi.com/v1/accounts/{self.account_id}/orders/{id}"
